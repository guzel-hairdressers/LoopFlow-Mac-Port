# LoopFlow | Rhino to Blender Sync (macOS & Windows Edition)
...
LoopFlow is a high-performance **Rhino 8 to Blender LiveSync Engine** built for macOS (Apple Silicon & Intel) and Windows. It provides zero-overhead geometry synchronization, instant layer visibility updates (< 0.06s), camera viewport mirroring, and light alignment.
...
- **Instant Fast Path Sync (< 0.06s)**: Pure metadata & layer visibility updates execute in under 60ms without re-reading 3DM geometry or re-welding meshes.
- **Native C++ Teardown (< 2.5s)**: Full scene reset of 20,000+ objects runs in native C++ via collection unlinking, bypassing Python O(N²) Dependency Graph rebuilds.
...
2. Open Blender 5.1/5.2 -> **Edit** -> **Preferences** -> **Add-ons** -> **Install from Disk**.
...
Every execution of **Model Sync** automatically appends structured suboperation performance and RAM footprint metrics (in JSONLines `.jsonl` format) to:
